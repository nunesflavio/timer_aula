...
export function Countdown() {
    ...
    // arredonda numero para baixo Math.floor
    const minutesAmount = Math.floor(currentSeconds / 60);
    const secondsAmount = currentSeconds % 60;
    // preenche uma string com um tamanho especifico com algum caracter
    const minutes = String(minutesAmount).padStart(2, '0');
    const seconds = String(secondsAmount).padStart(2, '0');
    // roda o tempo na aba do browser
    useEffect(() => {
        if (activeCycle) {
    ...
    useEffect(() => {
            ...
            if (activeCycle) {
                interval = setInterval(() => {
                    const secondsDiffrence = differenceInSeconds(new Date(), new Date(activeCycle.startDate));
